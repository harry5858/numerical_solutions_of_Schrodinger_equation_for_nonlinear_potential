...
for n=1:nx-2
    ...
    x(n+1) = x(n)+dx;
    k1=dx*(z(n));
    l1=dx*(-1*y(n)*(a+b*tanh((x(n)-6)/dT))^2);
    k2=dx*(z(n)+l1/2);
    l2=dx*(-1*(y(n)+k1/2)*(a+b*tanh((x(n+1)-6)/dT))^2);
    k3=dx*(z(n)+l2/2);
    l3=dx*(-1*(y(n)+k2/2)*(a+b*tanh((x(n+1)-6)/dT))^2);
    k4=dx*(z(n)+l3/2); 
    l4=dx*(-1*(y(n)+k3/2)*(a+b*tanh((x(n+2)-6)/dT))^2);
...
f_out_a=sqrt(1/(2*w_f(j)))*exp(-1i*w_f(j)*x(i));

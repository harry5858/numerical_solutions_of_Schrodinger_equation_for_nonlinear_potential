...
for n=2:nx-1
    I_next1(n)=I_initial(n)+(dt/(4*(dx^2)))*(R_initial(n+1)-2*R_initial(n)+R_initial(n-1))-(dt/2)*V(n)*R_initial(n);
    I_next1(1)=0;
    I_next1(nx)=0;
end
for n=2:nx-1
    R_next1(n)=R_initial(n)-(dt/(4*(dx^2)))*(I_initial(n+1)-2*I_initial(n)+I_initial(n-1))+(dt/2)*V(n)*I_initial(n);   
    R_next1(1)=0;
    R_next1(nx)=0;
end
R_current=R_next1;
...
%     error_real(i)=norm(R_current-psi_exact_r);
    ...
    error_real(i)=abs(R_current((nx/2)+1)-psi_exact_r((nx/2)+1));
...
plot(t,N)
hold on
title('normalization vs time','Fontsize', 24)
xlabel('t','Fontsize', 24)
ylabel('normalization','Fontsize', 24)
set(gca,'fontsize',20)
figure(4)
hold on
plot(t,error_real)
title('error of the real part of numerical psi vs time at a fixed point')
xlabel('t','Fontsize', 24)
